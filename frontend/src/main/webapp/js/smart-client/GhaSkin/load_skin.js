...
isc.loadSkin = function (theWindow) {
    ...
    with (theWindow) {
        ...
        isc.Page.setSkinDir("js/smart-client/GhaSkin/");
        isc.Page.loadStyleSheet("[SKIN]/skin_styles.css", theWindow);
        
        isc.Canvas.setProperties({
            // use synthetic scrollbars in mobile and for Macs, since from OSX
            // Lion onward,
        ...
        // define IButton so examples that support the new SmartClient skin
        // image-based
        ...
        if (isc.MenuButton) {
            ...
            isc.MenuButton.addProperties({
                // copy the header (.button) background-color to match when sort
                // arrow is hidden
                baseStyle: "button"
            });
        }
        ...
        isc.Layout.addProperties({
            resizeBarSize: 9,
            // Use the Snapbar as a resizeBar by default - subclass of Splitbar
            // that
            // shows interactive (closed/open) grip images
            // Other options include the Splitbar, StretchImgSplitbar or
            // ImgSplitbar
        ...
        if (isc.TreeGrid) {
            isc.TreeGrid.addProperties({
                ...
                openerIconSize: 22,
                sortAscendingImage: {
                    src: "[SKINIMG]ListGrid/sort_ascending.gif",
                    width: 7,
                    height: 7
                },
                sortDescendingImage: {
                    src: "[SKINIMG]ListGrid/sort_descending.gif",
                    width: 7,
                    height: 7
                }
            });
        ...
        if (isc.TabSet) {
            ...
            // In Netscape Navigator 4.7x, set the backgroundColor directly
            // since the css
            // background colors are not reliable
            if (isc.Browser.isNav) {
                isc.TabSet.addProperties({
                    paneContainerDefaults: {
            ...
            isc.TabBar.addProperties({
                ...
                // keep the tabs from reaching the curved edge of the pane
                // (regardless of align)
                layoutStartMargin: 5,
                layoutEndMargin: 5,
        ...
        // Native FILE INPUT items are rendered differently in Safari from other
        // browsers
        ...
        // Default EdgedCanvas skinning (for any canvas where showEdges is set
        // to true)
        if (isc.EdgedCanvas) {
        ...
        // remember the current skin so we can detect multiple skins being
        // loaded
        if (isc.setCurrentSkin)
            isc.setCurrentSkin("Simplicity");
